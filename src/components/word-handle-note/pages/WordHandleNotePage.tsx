/**
 * WordHandleNotePage Component
 * Displays notes accessed by word_handle, reusing NoteDetailPage logic
 */
...
import { useWordHandleNote } from '../hooks/useWordHandleNote';

interface WordHandleNotePageProps {
  wordHandle: string;
  className?: string;
}

export const WordHandleNotePage: React.FC<WordHandleNotePageProps> = ({ 
  wordHandle, 
  className = '' 
}) => {
  const { note, isLoading, error, hasAccess, checkingAccess, user, refreshAccess } = useWordHandleNote(wordHandle);
        ...
        <main className="flex-grow container mx-auto px-4 py-8">
          <div className="max-w-4xl mx-auto">
            <div className="text-center py-12">
              ...
              <p className="text-muted-foreground">
                {error || `No note found with handle "${wordHandle}"`}
              </p>
            </div>
          </div>
        </main>
...
export default WordHandleNotePage;