...
const PhoneIcon = Phone as React.ElementType;
...
const LocalAuthModal = ({ isOpen, onClose, initialMode }: LocalAuthModalProps) => {
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4">
        ...
        <div className="relative w-full max-w-sm sm:max-w-md my-4 sm:my-8 mx-auto">
          <div className="relative flex flex-col bg-gradient-to-br from-ghost-dark via-ghost-gray to-ghost-dark border border-ghost-purple/30 rounded-xl sm:rounded-2xl shadow-2xl shadow-ghost-purple/20 animate-in zoom-in-95 slide-in-from-bottom-5 duration-300 max-h-[90vh] overflow-y-auto">
            ...
            >
              <XIcon className="w-5 h-5 sm:w-6 sm:h-6 text-gray-400 hover:text-white" />
            </button>
            ...
            {mode !== 'private' ? (
              <form onSubmit={handleSubmit} className="px-6 sm:px-8 pb-4 sm:pb-6 space-y-3 sm:space-y-4">
                ...
                <div className="relative">
                  <MailIcon className="absolute left-3 sm:left-4 top-1/2 -translate-y-1/2 w-5 h-5 sm:w-5 sm:h-5 text-gray-400" />
                  <input
                    type="email"
                    placeholder={mode === 'login' ? 'Email or Username' : 'Email'}
                    value={formData.email}
                    onChange={(e) => handleInputChange('email', e.target.value)}
                    className="w-full pl-10 sm:pl-12 h-11 sm:h-12 bg-ghost-gray/50 border border-white/10 text-white placeholder:text-gray-400 focus:border-white/20 focus:bg-ghost-gray/80 focus:outline-none focus:ring-0 text-sm sm:text-base rounded-md"
                  />
                </div>
                ...
                {mode === 'signup' && (
                  <div className="relative">
                    <UserIcon className="absolute left-3 sm:left-4 top-1/2 -translate-y-1/2 w-5 h-5 sm:w-5 sm:h-5 text-gray-400" />
                    <input
                      type="text"
                ...
                {/* Password */}
                <div className="relative">
                  <LockIcon className="absolute left-2.5 sm:left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 sm:w-4 sm:h-4 text-gray-400" />
                  <input
                    type={showPassword ? 'text' : 'password'}
                    placeholder="Password"
                    value={formData.password}
                    onChange={(e) => handleInputChange('password', e.target.value)}
                    className="w-full pl-8 sm:pl-10 pr-8 sm:pr-10 h-9 sm:h-10 bg-ghost-gray/50 border border-white/10 text-white placeholder:text-gray-400 focus:border-white/20 focus:bg-ghost-gray/80 focus:outline-none focus:ring-0 text-xs sm:text-sm rounded-md"
                  />
                  <button
                    type="button"
                  ...
                  >
                    {showPassword ? <EyeOffIcon className="w-3.5 h-3.5 sm:w-4 sm:h-4" /> : <EyeIcon className="w-3.5 h-3.5 sm:w-4 sm:h-4" />}
                  </button>
                </div>
                ...
                {mode === 'signup' && (
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="w-full pl-8 sm:pl-10 h-9 sm:h-10 bg-ghost-gray/50 border border-white/10 text-white placeholder:text-gray-400 focus:border-white/20 focus:bg-ghost-gray/80 focus:outline-none focus:ring-0 text-xs sm:text-sm rounded-md"
                    />
                  </div>
                )}
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex items-center space-x-2">
                    <input
                      ...
                      readOnly
                      className="flex-1 h-10 px-3 bg-ghost-gray/50 border border-white/10 text-white font-mono focus:border-white/20 focus:bg-ghost-gray/80 focus:outline-none focus:ring-0 rounded-md"
                    />
                    <Button
                      variant="outline"
                      size="icon"
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    <input
                      ...
                      className="w-full pr-10 h-10 px-3 bg-ghost-gray/50 border border-white/10 text-white placeholder:text-gray-400 focus:border-white/20 focus:bg-ghost-gray/80 focus:outline-none focus:ring-0 rounded-md"
                    />
                    <button
                      type="button"
            ...
            <div className="px-6 sm:px-8 pb-6 sm:pb-8 flex justify-center space-x-3 sm:space-x-4">
              ...
              <Button
                ...
                title="Private Account (Anonymous)"
              >
