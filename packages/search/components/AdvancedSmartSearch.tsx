...
import { Slider } from 'packages/ui-components/components/slider';
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from 'packages/ui-components/components/select';
...
const AdvancedSmartSearch = ({ 
  ...
  const getSemanticScore = useCallback((searchQuery: string, note: Note): number => {
    const queryLower = searchQuery.toLowerCase();
    const keywords = queryLower.split(' ').filter(word => word.length > 1);

    const semanticMap: { [key: string]: { synonyms: string[], weight: number } } = {
      'motivation': { synonyms: ['motivational', 'inspiration', 'encouragement', 'uplift', 'positive'], weight: 1.0 },
      'breakup': { synonyms: ['relationship', 'love', 'heart', 'emotional', 'dating'], weight: 1.0 },
      'dark': { synonyms: ['gothic', 'noir', 'mysterious', 'shadow', 'thriller'], weight: 0.8 },
      'writing': { synonyms: ['content', 'text', 'author', 'creative', 'copywriting'], weight: 0.9 },
      'startup': { synonyms: ['business', 'entrepreneur', 'pitch', 'company', 'venture'], weight: 1.0 },
      'interview': { synonyms: ['job', 'career', 'professional', 'coding', 'technical'], weight: 1.0 },
      'ai': { synonyms: ['artificial', 'intelligence', 'machine', 'learning', 'neural'], weight: 1.0 },
      'design': { synonyms: ['ui', 'ux', 'visual', 'interface', 'user'], weight: 0.9 },
      'security': { synonyms: ['cyber', 'protection', 'hack', 'safe', 'privacy'], weight: 1.0 },
      'react': { synonyms: ['javascript', 'frontend', 'component', 'jsx', 'hooks'], weight: 1.0 },
      'advanced': { synonyms: ['expert', 'professional', 'complex', 'sophisticated'], weight: 0.8 }
    };
    ...
    keywords.forEach(keyword => {
      ...
      Object.entries(semanticMap).forEach(([key, data]) => {
        if (keyword.includes(key) || data.synonyms.some(syn => keyword.includes(syn))) {
  ...
  const calculateAdvancedScore = useCallback((note: RankedNote, contentScore: number): RankedNote => {
    ...
    const purchaseScore = Math.min(1.0, Math.log(1 + mockMetrics.purchase_count) / Math.log(101));
    const viewScore = Math.min(1.0, Math.log(1 + mockMetrics.view_count) / Math.log(1001));
    const ratingScore = note.rating ? (note.rating / 5.0) * Math.min(1.0, (note.reviews || 0) / 10.0) : 0;
    const popularity_score = (purchaseScore * 0.5) + (ratingScore * 0.3) + (viewScore * 0.2);
  ...
  const performAdvancedSearch = useCallback((searchQuery: string) => {
    if (!searchQuery.trim()) {
      setResults([]);
      setFallbackResults([]);
      setIsLoading(false);
    ...
    let filteredNotes = selectedCategory === 'All' 
      ? sampleNotes 
    ...
    const qualityResults = scoredResults.filter(result => result.content_similarity! > 0);

    // Sort based on selected criteria
    let sortedResults: RankedNote[];
    switch (sortBy) {
      case 'popularity':
        sortedResults = qualityResults.sort((a, b) => (b.popularity_score || 0) - (a.popularity_score || 0));
        break;
      case 'recency':
    ...
    if (sortedResults.length < 3 || (sortedResults[0]?.final_score || 0) < 0.4) {
    ...
    setIsLoading(false);
  }, [selectedCategory, minRating, maxPrice, sortBy, getSemanticScore, calculateAdvancedScore]);
  ...
  return (
    <Card className={`absolute top-full mt-2 w-full bg-ghost-dark/95 backdrop-blur-lg border-ghost-purple/30 shadow-2xl shadow-ghost-purple/10 z-50 max-h-[600px] overflow-y-auto ${className}`}>
      <div className="p-4">
        ...
        {isLoading && (
          <div className="flex items-center justify-center py-8">
            ...
            <span className="text-gray-300">Analyzing with advanced AI ranking...</span>
          </div>
        )}
                ...
                >
                  <div className="flex justify-between items-center">
                    <div>
                      ...
                      <p className="text-sm text-gray-400">{note.category} • {note.author}</p>
                    </div>
        ...
        {/* No Results */}
        {!isLoading && query.length > 2 && results.length === 0 && fallbackResults.length === 0 && (
