...
export function ReaderHeader({ 
  ...
  return (
    <header className={`sticky top-0 z-50 reader-header transition-colors duration-300 ${
      colorScheme === 'dark' ? 'bg-ghost-black border-gray-800' :
      colorScheme === 'sepia' ? 'bg-amber-50 border-amber-200' :
          ...
          <div className="relative sm:hidden">
            ...
            {showOptionsMenu && (
              <div className={`absolute right-0 top-full mt-2 w-48 rounded-md shadow-lg z-50 ${
                colorScheme === 'dark' ? 'bg-gray-800 border-gray-700' :
                colorScheme === 'sepia' ? 'bg-amber-50 border-amber-200' :
                'bg-white border-gray-200'
              } border`}>
                <div className="py-1">
                    ...
                    <div className="flex space-x-2">
                      <button
                        ...
                        className={`px-2 py-1 text-xs rounded ${
                          colorScheme === 'light' ? 'bg-blue-500 text-white' : 'bg-gray-200 text-gray-700'
                        }`}
                      >
                        Light
                      </button>
                      <button
                        onClick={() => onColorSchemeChange('sepia')}
                        className={`px-2 py-1 text-xs rounded ${
                          colorScheme === 'sepia' ? 'bg-amber-500 text-white' : 'bg-gray-200 text-gray-700'
                        }`}
                      >
                        Sepia
                      </button>
                      <button
                        onClick={() => onColorSchemeChange('dark')}
                        className={`px-2 py-1 text-xs rounded ${
                          colorScheme === 'dark' ? 'bg-gray-700 text-white' : 'bg-gray-200 text-gray-700'
                        }`}
                      >
                        Dark
                      </button>
