...
export async function saveNote(noteData: NoteFormData): Promise<SaveNoteResult> {
  try {
    ...
    const { data, error } = await supabase
      ...
      .insert({
        ...
        word_count: noteData.content.split(/\s+/).filter(word => word.length > 0).length,
...
export async function updateNote(noteId: number, noteData: NoteFormData): Promise<SaveNoteResult> {
  try {
    const { data: { user } } = await supabase.auth.getUser();
